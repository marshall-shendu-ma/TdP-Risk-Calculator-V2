  ...
  document.getElementById("riskForm").addEventListener("submit", e => {
    ...
    hillChart=new Chart(document.getElementById("hillPlot"),{
      ...
      data:{labels:fitX,datasets:[
        {label:"Hill Fit Curve",data:fitX.map((x,i)=>({x,y:fitY[i]})),borderWidth:2,fill:false,tension:0.1,borderColor:"#0071bc"},
        {label:"Input Data",data:concs.map((x,i)=>({x:x,y:fpdcs[i]})),type:"scatter",pointBackgroundColor:"#4da6ff",pointRadius:5},
        {label:"Cmax Point",data:[{x:Cmax,y:FPD_Cmax}],type:"scatter",pointBackgroundColor:"#ff6b6b",pointRadius:6}
      ]},
