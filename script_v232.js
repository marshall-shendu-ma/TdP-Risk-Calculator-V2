document.addEventListener("DOMContentLoaded", function() {
  ...
  // Compute risk directly from predictor inputs
  function computeRiskFromPredictors() {
    ...
    const logit1 = -0.1311 + map1[p1] + 0.00687 * p4 + 0.0232 * p7;
    Prob1 = 1/(1+Math.exp(-logit1));
    // Model 2 ordinal regression
    const logit2a = -0.1211 + cellVal * 0.2211 + map2a[p1] + 0.00105 * p4 + 0.0338 * p7;
    Prob2a = 1/(1+Math.exp(-logit2a));
    const logit2b = -2.1102 + cellVal * 0.2211 + map2b[p1] + 0.00105 * p4 + 0.0338 * p7;
    Prob2b = 1/(1+Math.exp(-logit2b));
    isModel1 = true;
    updateModelPanel();
  }
  // Bind predictor calculate button inside DOMContentLoaded
  document.getElementById("predictorCalcBtn").addEventListener("click", function(e) {
    e.preventDefault();
    computeRiskFromPredictors();
  });
